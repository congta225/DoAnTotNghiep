...
class VendorWithdrawController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->amount < $method->minimum_amount || $request->amount > $method->maximum_amount){
           throw ValidationException::withMessages(["Số tiền phải lớn hơn $method->minimum_amountvà và ít hơn $method->maximum_amount"]);
        }
        ...
        if($request->amount > $currentBalance){
            throw ValidationException::withMessages(['Thiếu cân bằng']);
        }

        if(WithdrawRequest::where(['vendor_id' => auth()->user()->id, 'status' => 'Chờ'])->exists()){
            throw ValidationException::withMessages(['Bạn đã có một yêu cầu đang chờ xử lý']);
        }
        ...
        toastr('Đã thêm yêu cầu thành công');
