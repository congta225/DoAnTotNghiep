        ...
        <form action="{{route('admin.paypal-setting.update', 1)}}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="status" id="" class="form-control">
                    <option {{$paypalSetting->status === 1 ? 'selected' : ''}} value="1">Cho phép</option>
                    <option {{$paypalSetting->status === 0 ? 'selected' : ''}} value="0">Vô hiệu hóa</option>
                </select>
            </div>
            ...
            <div class="form-group">
                <label>Tỷ giá tiền tệ ( Mỗi 1.000 {{$settings->currency_name}} )</label>
                <input type="text" class="form-control" name="currency_rate" value="{{$paypalSetting->currency_rate}}">
            </div>
            <div class="form-group">
                <label>Id máy chủ Paypal</label>
