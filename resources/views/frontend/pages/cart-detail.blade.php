            ...
            <div class="row">
                                ...
                                <tbody>
                                    <tr class="d-flex">
                                        ...
                                        <th class="wsus__pro_icon">
                                            <a href="#" class="common_btn clear_cart">Xóa tất cả</a>
                                        </th>
                                    </tr>
                                    ...
                                    <tr class="d-flex">
                                        ...
                                        <td class="wsus__pro_name">
                                            ...
                                            @foreach ($item->options->variants as $key => $variant)
                                                <span>{{$key}}: {{$variant['name']}} ({{number_format($variant['price'],0,'.','.').$settings->currency_icon}})</span>
                                            @endforeach

                                        </td>

                                        <td class="wsus__pro_tk">
                                            <h6>{{number_format($item->price,0,'.','.').$settings->currency_icon}}</h6>
                                        </td>

                                        <td class="wsus__pro_tk">
                                            <h6 id="{{$item->rowId}}">{{number_format((($item->price + $item->options->variants_total) * $item->qty),0,'.','.').$settings->currency_icon}}</h6>
                                        </td>
                ...
                <div class="col-xl-3">
                    <div class="wsus__cart_list_footer_button" id="sticky_sidebar">
                        ...
                        <p>Tổng cộng: <span id="sub_total">{{number_format(getCartTotal(),0,'.','.')}}{{$settings->currency_icon}}</span></p>
                        <p>Mã giảm giá(-): <span id="discount">{{number_format(getCartDiscount(),0,'.','.')}}{{$settings->currency_icon}}</span></p>
    ...
    $(document).ready(function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status === 'success'){
                        let productId = '#'+rowId;
                        let totalAmount =  numeral(data.product_total ).format('0,0') + "{{$settings->currency_icon}}"
        ...
        function renderCartSubTotal(){
            $.ajax({
                ...
                success: function(data) {
                    $('#sub_total').text(numeral(data).format('0,0') + "{{$settings->currency_icon}}");
                },
                error: function(data) {
                    console.log(data);
                }
        ...
        function calculateCouponDescount(){
            $.ajax({
                ...
                success: function(data) {
                    if(data.status === 'success'){
                        $('#discount').text(numeral(data.discount).format('0,0') + '{{$settings->currency_icon}}');
                        $('#cart_total').text(numeral(data.cart_total).format('0,0') +'{{$settings->currency_icon}}');
                    }
                },
                error: function(data) {
                    console.log(data);
