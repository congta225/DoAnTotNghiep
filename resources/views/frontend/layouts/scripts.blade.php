<script>
    $(document).ready(function() {
                    ...
                    for (let item in data) {
                        ...
                        <li id="mini_cart_${product.rowId}">
                            ...
                            <div class="wsus__cart_text">
                                ...
                                <p>` + numeral(product.price).format('0,0') + `{{ $settings->currency_icon }}</p>
                                <small>Tổng biến thể: ` + numeral(product.options.variants_total).format('0,0') + `{{ $settings->currency_icon }}</small>
                                <br>
        ...
        // Nhận tổng tiền giỏ hàng bên sidebar
        function getSidebarCartSubtoal() {
            $.ajax({
                method: 'GET',
                url: "{{ route('cart.sidebar-product-total') }}",
                success: function(data) {
                    $('#mini_cart_subtotal').text(numeral(data).format('0,0') +"{{ $settings->currency_icon }}");
                },
                error: function(data) {
